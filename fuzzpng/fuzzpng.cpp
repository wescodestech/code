...
void ReadPngFile( char* filename )
{
        ...
        intoPtr = png_create_info_struct( pngPtr );
        if( !intoPtr )
		{
        ...
        png_read_info( pngPtr, intoPtr );

        width = png_get_image_width( pngPtr, intoPtr );
        height = png_get_image_height( pngPtr, intoPtr );
        colorType = png_get_colorType( pngPtr, intoPtr );
        bitDepth = png_get_bitDepth( pngPtr, intoPtr );

        numOfPasses = png_set_interlace_handling( pngPtr );
        png_read_update_info( pngPtr, intoPtr );
        ...
        for( y=0; y < height; ++y )
		{
			rowPointers[y] = (png_byte*) malloc( png_get_rowbytes( pngPtr,intoPtr ) );
		}
...
void WritePngFile( char* filename )
{
        ...
        png_set_IHDR( pngPtr, intoPtr, width, height,
                     bitDepth, colorType, PNG_INTERLACE_NONE,
                     PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE );

        png_write_info( pngPtr, intoPtr );
